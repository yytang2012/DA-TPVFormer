...
@hydra.main(
    version_base=None,               # 1.2/1.3 推荐写法，消除“Will assume defaults for 1.1”
    config_path="../../config",       # 配置目录（相对当前 .py 文件路径）
    config_name="kitti_ssc.yaml"      # 配置文件名
)
def main(config: DictConfig):
    ...
    for sequence in sequences:
        ...
        downscaling = {"1_1": 1, "1_8": 8, "1_2": 2, "1_4": 4}
        ...
        for i in tqdm(range(len(label_paths))):
            ...
            for scale in downscaling:
                ...
                if not os.path.exists(label_filename):
                    if scale == "1_8" or scale == "1_2" or scale == "1_4":
