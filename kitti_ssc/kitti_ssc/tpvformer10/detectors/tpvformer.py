...
from config.tpv_lidarseg_dim64 import tpv_h_
...
class TPVFuser(BaseModule):
    ...
    def forward(self, bev_list, points=None, mode='low'):
        """
        bev_list[0]: bs, h*w, c
        bev_list[1]: bs, z*h, c
        bev_list[2]: bs, w*z, c
    ...
    def forward_h(self, bev_list_h, bev_list, points=None):
        """
        bev_list[0]: bs, h*w, c
        bev_list[1]: bs, z*h, c
        bev_list[2]: bs, w*z, c
        ...
        bev_hw_h, bev_zh_h, bev_zw_h = bev_list_h[0], bev_list_h[1], bev_list_h[2]
        bev_hw_h = bev_hw_h + bev_hw_h.mean(dim=1, keepdim=True) * 0.0
        bev_zh_h = bev_zh_h + bev_zh_h.mean(dim=1, keepdim=True) * 0.0
        bev_zw_h = bev_zw_h + bev_zw_h.mean(dim=1, keepdim=True) * 0.0
        bs, _, c = bev_hw_h.shape
        bev_hw_h = bev_hw_h.permute(0, 2, 1).reshape(bs, c, self.bev_h, self.bev_w)
        bev_zh_h = bev_zh_h.permute(0, 2, 1).reshape(bs, c, self.bev_z, self.bev_h)
        bev_zw_h = bev_zw_h.permute(0, 2, 1).reshape(bs, c, self.bev_w, self.bev_z)
        ...
        if scale_h != 1 or scale_w != 1:
            bev_hw_h = F.interpolate(
                bev_hw_h,
                size=(self.bev_h * scale_h, self.bev_w * scale_w),
                # scale_factor=self.scale_h,
                mode='bilinear'
            )
        if scale_z != 1 or scale_h != 1:
            bev_zh_h = F.interpolate(
                bev_zh_h,
                size=(self.bev_z * scale_z, self.bev_h * scale_h),
                # scale_factor=self.scale_w,
                mode='bilinear'
            )
        if scale_w != 1 or scale_z != 1:
            bev_zw_h = F.interpolate(
                bev_zw_h,
                size=(self.bev_w * scale_w, self.bev_z * scale_z),
                # scale_factor=self.scale_z,
                mode='bilinear'
            )
        ...
        if points is not None:
            ...
            if self.use_checkpoint:
                ...
                logits = self.classifier(fused)
            logits = logits.permute(0, 2, 1)
            logits_vox = logits[:, :, :(-n)].reshape(bs, self.classes, self.scale_w * self.bev_w,
                                                     self.scale_h * self.bev_h, self.scale_z * self.bev_z)
            logits_pts = logits[:, :, (-n):].reshape(bs, self.classes, n, 1, 1)
            return logits_vox, logits_pts
            ...
            bev_hw_h = bev_hw_h.unsqueeze(-1).expand(-1, -1, -1, -1, scale_z * self.bev_z)
            bev_zh_h = bev_zh_h.unsqueeze(-1).permute(0, 1, 3, 4, 2).expand(-1, -1, -1, scale_w * self.bev_w, -1)
            bev_zw_h = bev_zw_h.unsqueeze(-1).permute(0, 1, 4, 2, 3).expand(-1, -1, scale_h * self.bev_h, -1, -1)
            fused_h = bev_hw_h + bev_zh_h + bev_zw_h
            fused_l = bev_hw + bev_zh + bev_zw
            fused = fused_l[:, :, 32:96, 32:96, 4:12] * 0.25 + fused_h * 0.75
...
class TPVFormer(BaseModule):
    ...
    def __init__(self,
        ...
        if pts_bbox_head:
            ...
            pts_bbox_head_high = copy.deepcopy(pts_bbox_head)
            pts_bbox_head_high.pc_range = [12.8, -12.8, -0.4, 38.4, 12.8, 2.8]
            # pts_bbox_head_high.bev_h = tpv_h_
            # pts_bbox_head_high.bev_w = tpv_w_
            # pts_bbox_head_high.bev_z = tpv_z_
            self.pts_bbox_head_h = builder.build_head(pts_bbox_head_high)
    ...
    def forward_pts_train(self,
        ...
        """
        outs_h = self.pts_bbox_head_h(pts_feats, img_metas, prev_bev)
    ...
    def forward_train(self,
        ...
        if hasattr(self, 'img_backbone'):
            ...
            img_feats = img
        outs_h, outs = self.forward_pts_train(img_feats, gt_bboxes_3d,
                                      gt_labels_3d, img_metas,
                                      gt_bboxes_ignore)
        # outs_h = self.fusion_head.forward_h(outs_h, outs, points)
        outs_h = self.fusion_head(outs_h, points, mode='high')
