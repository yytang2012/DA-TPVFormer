...
class TPV(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        out_h, out = self.net_3d_decoder(**input_dict)

        return {'ssc_logit_h': out_h, 'ssc_logit': out}
    ...
    def step(self, batch, step_type, metric):
        ...
        ssc_pred_h = out_dict["ssc_logit_h"]
        target = batch["target"]

        # target_h = target[:, 32:96, 32:96, 4:12]
        target_h = target[:, 64:192, 64:192, 8:24]
        ...
        if self.CE_ssc_loss:
            ...
            loss_ssc_h = CE_ssc_loss(ssc_pred_h, target_h, class_weight)
            loss_h += loss_ssc_h
            loss_l += loss_ssc
            self.log(
                step_type + "/loss_ssc",
        ...
        if self.sem_scal_loss:
            ...
            loss_l += loss_sem_scal
            loss_sem_scal_h = sem_scal_loss(ssc_pred_h, target_h)
            loss_h += loss_sem_scal_h
            self.log(
            ...
            self.log(
                step_type + "/loss_sem_scal_h",
                loss_sem_scal_h.detach(),
                on_epoch=True,
                sync_dist=True,
            )
        ...
        if self.geo_scal_loss:
            ...
            loss_l += loss_geo_scal
            loss_geo_scal_h = geo_scal_loss(ssc_pred_h, target_h)
            loss_h += loss_geo_scal_h
            self.log(
            ...
            self.log(
                step_type + "/loss_geo_scal_h",
                loss_geo_scal_h.detach(),
                on_epoch=True,
                sync_dist=True,
            )
        ...
        y_pred_h = ssc_pred_h.detach().cpu().numpy()
        # y_pred[:, :, 32:96, 32:96, 4:12] = y_pred_h
        y_pred[:, :, 64:192, 64:192, 8:24] = y_pred_h
        ...
        self.log(step_type + "/loss_l", loss_l.detach(), on_epoch=True, sync_dist=True)
        self.log(step_type + "/loss_h", loss_h.detach(), on_epoch=True, sync_dist=True)
        loss += loss_l + loss_h
